...
// function to place random mines on the game grid along with mine numbers
...
void Game::clearEverything()
{
    ...
    this->mines.clear();
    if(this->grid){
        for(int i=0; i < this->difficulties[this->current_difficulty].grid_height+2; i++){
            for(int j=0; j < this->difficulties[this->current_difficulty].grid_width+2; j++){
                this->grid[i][j]=-1;
            }
        }
    }
...
void Game::createInvisibleGrid(int rows, int cols)
{
...
void Game::freeInvisibleGrid(int rows)
{
    for(int i=0;i<rows;i++){
        delete [] this->grid[i];
    }
...
int **Game::getGrid() const
{
    return grid;
}

void Game::setGrid(int **value)
{
    grid = value;
}
